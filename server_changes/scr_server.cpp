...
drive(int index, tCarElt* car, tSituation *s)
{
	...
	float leaderDistance, distFromLeader;
	
	if (car->race.pos > 1 || s->_ncars == 1){
		//if this car is not the leader or there is only this car, then we consider the first car
		leaderDistance = s->cars[0]->_distRaced;
	}else{
		//otherwise, we consider the distance from the first car behind this one
		leaderDistance = s->cars[1]->_distRaced;
	}
	distFromLeader = car->_distRaced - leaderDistance;
	...
	stateString += SimpleParser::stringify("distFromLeader", distFromLeader);//ML
