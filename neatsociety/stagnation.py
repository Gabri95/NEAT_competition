...
class PreserveBestStagnation(DefaultStagnation):
    
    #prevent the stagnation to remove the species with the best genome found in the current generation
    
    def __init__(self, config, reporters):
        super(PreserveBestStagnation, self).__init__(config, reporters)
    
    def update(self, species):
        result = []
        
        best_species = max([(s.ID, max([m.fitness for m in s.members])) for s in species], key=lambda x: x[1])[0]
        ...
        for s in species:
            ...
            is_stagnant = scount >= self.max_stagnation and s.ID != best_species
            result.append((s, is_stagnant))
